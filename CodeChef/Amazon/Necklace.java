 ...
 * Problem:https://www.codechef.com/practice/course/amazon-interview-questions/AMAZONPREP/problems/NEC
 * Name:Next
 * Necklace
 */
import java.util.*;
...
public class Necklace{
    ...
    public static void main(String args[]) throws IOException {
        ...
        while(t-->0) {
            ...
            k=k%n;
            if(k==0) {arrprint(arr);continue;}
            int arr1[]=new int[n];
            int j=0;
            for(int i=k;i<n;i++){
                arr1[j++]=arr[i];
            }
            for(int i=0;i<k;i++) arr1[j++]=arr[i];
            arrprint(arr1);
            res.append("\n");
        }
        //print(res);
