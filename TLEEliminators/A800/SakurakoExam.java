...
public class SakurakoExam{
    ...
    public static void main(String args[]) throws IOException {
        ...
        while(t-->0){
            int n= sc.nextInt();
            int m= sc.nextInt();
            if(n%2==1) res.append("NO");//If 1 count odd
            else {//If one count 0+even
                if(m%2==0) res.append("YES");//if 1 and 2 count 0+even
                else{//if 2 count odd and 1 count 0+even
                    if(n==0) res.append("NO");
                    else res.append("YES");//Both count even>0
                }
            }
            res.append("\n");
        }
